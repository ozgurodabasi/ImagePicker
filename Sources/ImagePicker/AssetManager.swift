...
open class AssetManager {
    ...
    let ali = PHFetchOptions()
    ali.includeAssetSourceTypes = .typeUserLibrary
    
    DispatchQueue.global(qos: .background).async {
      let fetchResult = configuration.allowVideoSelection
        ? PHAsset.fetchAssets(with: ali)
        : PHAsset.fetchAssets(with: .image, options: ali)
