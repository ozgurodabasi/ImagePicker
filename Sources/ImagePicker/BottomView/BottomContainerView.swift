...
open class BottomContainerView: UIView {
    ...
    lazy var pickerButton: ButtonPicker = { [weak self] in
        guard let self = self else { return ButtonPicker() }
        let pickerButton = ButtonPicker(configuration: self.configuration)
        pickerButton.isEnabled = false
        // gecikme memory icin kullaniliyor,
        Timer.scheduledTimer(timeInterval: 1.2, target: self, selector: #selector(doSomeStuff), userInfo: nil, repeats: false)
